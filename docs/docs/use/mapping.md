...
ChmlFrp提供图形化客户端（ChmlFrpLauncher），支持Windows、macOS和Linux系统，提供统一的用户界面和操作体验。
...
![图形客户端下载页面](./img/mapping/gui-download.webp)

2. **安装客户端**
   - **Windows**: 下载两个安装程序随意下载，双击运行安装
   - **macOS**: 下载文件，拖拽到应用程序文件夹（仅支持M系列芯片）
   - **Linux**: 下载.AppImage文件，赋予执行权限后运行

### 登录和配置

1. **启动客户端**
   - 双击桌面图标或从应用程序菜单启动ChmlFrpLauncher
   - 启动客户端后，点击菜单中的"未登录"选项，登录账户

![客户端登录界面](./img/mapping/gui-login.webp)

2. **账号登录**
   - 点击授权登录，将跳转到浏览器登录，在浏览器中确认授权（浏览器需登录轻爪账户并授权）

3. **查看隧道列表**
   - 登录成功后自动显示您的所有隧道
   - 隧道信息包括名称、状态、节点、内外网地址等

![隧道列表界面](./img/mapping/gui-tunnel-list.webp)
...
> **💡 提示**  
> 我们更建议使用图形客户端启动，除非您因为一些原因无法使用图形客户端。
